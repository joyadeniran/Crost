...
### Session paused (mid-phase), then resumed and completed — founder's call
First paused after the atomic-claim fix and structured logging (see below for why). Founder later directed: "go back and complete phase 3" — the two remaining items, `lib/state-machine.ts` and worker retry/dead-letter/reaper, done in a follow-up continuation of this same phase.

**Why it was paused originally:** both remaining items were large investigative efforts — the status vocab across goals/tasks/approvals/artifacts was inconsistent and scattered across ~60 files (a *characterization-first* state machine needs real per-entity investigation, not an aspirational transition table), and `scripts/worker.ts` is a live polling process with zero test coverage this sandbox cannot execute (separate root-level package, no shared tsconfig/test runner, no zod).

### `lib/state-machine.ts` — characterization-based transition tables
Investigated the real status vocab per entity before writing anything (full grep of every `.from('goals'|'goal_tasks'|'approval_queue'|'artifacts').update/insert({status:...})` call across `app/api/**`, `lib/engine/**`, `lib/tools/**`, and the legacy Deno `expire-approvals` edge function), cross-checked against `cloudsql_migration.sql`'s CHECK constraints and the `artifacts` table's `enforce_artifact_status_transition()` DB trigger (the only entity with real DB-level transition enforcement today).

**Found and fixed a real, confirmed-live production bug in the process:** `goals_status_check` was missing `'clarifying'` and `'error'`, both written by `lib/engine/orchestrator.ts` in normal (non-exceptional) code paths — dialogue-mode clarification and the hallucination-retry-exhausted path respectively. Confirmed directly against the live Cloud SQL DB (`SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conname = 'goals_status_check'` returned the narrower 6-value list, exactly matching the file — not just doc drift, a live gap). `'clarifying'` had been added once before, in the pre-Cloud-SQL Supabase era (`supabase/migrations/20240101000014_dialogue_mode.sql`), but that widening was never carried into `cloudsql_migration.sql` at the GCP migration. `'error'` had never been added anywhere — a net-new, always-broken path. Fixed via `cloudsql_fixes_v13.17.sql`, founder-applied to the live DB and confirmed ("Statement executed successfully").

`lib/state-machine.ts`: `isValidGoalTransition`/`isValidGoalTaskTransition`/`isValidApprovalTransition`/`isValidArtifactTransition` + terminal/immutable-status helpers, one transition table per entity, every edge sourced from an actual code call site (documented inline). Flags two open gaps found along the way, not fixed in this file: `goal_tasks`/`approval_queue`'s `'expired'` status is only reachable via the legacy Deno `expire-approvals` edge function with no Next.js/Cloud Run equivalent found; `approval_queue` has two independent expiry mechanisms (`app/api/approvals/expire` cron route vs. the same Deno function) of unconfirmed relative liveness. Additive only — not wired into any route this session, so zero response-shape risk.

New tests: `tests/unit/state-machine.test.ts` (33) — one describe block per entity, asserting both the "should be legal" edges (grep-verified against real call sites) and "should be illegal" edges (e.g. `goal_tasks`' `approved`/`dispatched`/`needs_data` values exist in the DB CHECK constraint but have no observed writer anywhere in the codebase — characterized as unreachable, not guessed at).

### `scripts/worker.ts` — bounded retries, dead-letter, reaper + a real dispatch bug fix
Found a second real, confirmed production bug while reading the file end-to-end before changing it: `dispatchTask()` referenced a bare, never-declared `SUPABASE_SERVICE_KEY` identifier — a `ReferenceError` thrown synchronously on every call, silently swallowed by `pollSupervisor`'s outer `try/catch`. Net effect: the worker's entire auto-dispatch path (no-dependency pending tasks, dependency-unblocked tasks) has been dead in production since whenever that line was introduced, with the system relying entirely on approval-decision chain-reactions to make progress. Fixed to read `WORKER_INTERNAL_SECRET`/`SUPABASE_SERVICE_ROLE_KEY` — the same fallback the dispatch route's own auth guard expects — and fails fast at boot (matching the existing `DATABASE_URL` check) if neither is set.

`cloudsql_fixes_v13.18.sql`: `goal_tasks.retry_count` (int, default 0), `goal_tasks.next_retry_at` (timestamptz, nullable), and `'failed_permanent'` added to `goal_tasks_status_check` as the dead-letter status. Founder-applied to the live DB.

New `reapStaleTasks()`: tasks stuck in `'running'` past `STALE_RESET_THRESHOLD_MS` (10m) are requeued to `'pending'` with exponential backoff (30s/60s/120s) up to `MAX_TASK_RETRIES` (3), then dead-lettered to `'failed_permanent'` — replaces the old unconditional, unbounded reset-to-pending-forever behavior that ran once at boot only. The reaper now runs every poll cycle (previously boot-only, so a task that stalled mid-run while the worker was already up and healthy was never reaped until the next restart). `unblockDependentTasks` and the no-dependency pending-task dispatch loop both now respect `next_retry_at` before dispatching. `tryCloseGoal`'s terminal-status set and outcome summary updated for `failed_permanent`.

Not coverable by automated tests (separate root-level package, no test runner) — verified via `tsc --noEmit` (clean, via `frontend/node_modules/.bin/tsc` against the standalone file) plus full manual read, then **founder ran the live worker against the real Cloud SQL DB and the reaper end-to-end**: connected cleanly with no `SUPABASE_SERVICE_KEY is not defined` errors (confirming the dispatch-bug fix); manually forced a stale `running` task via Cloud SQL Studio and confirmed the reap-and-requeue log line with correct backoff; confirmed the `next_retry_at` gate correctly skipped early redispatch on the very next cycle; then forced `retry_count=3` + stale `assigned_at` again and confirmed dead-letter (`"dead-lettered after 3 retries"`) fired instead of another retry. All three new behaviors proven live, not just type-checked.

### Phase 3 — EXIT GATE STATUS: MET
Per `docs/DEVELOPMENT_PLAN_10X.md`: "T8 upgraded tests green; a simulated double-dispatch test proves no duplicate execution; full suite green."
- Double-dispatch: proven no duplicate execution (atomic claim fix, earlier this phase).
- State machine + worker retry/dead-letter/reaper: both complete, founder-verified live.
- Suite green: `tsc --noEmit` clean across all changed files; state-machine tests founder-confirmed green locally.

All items from the original Phase 3 scope are now closed: atomic dispatch claim, structured logging (engine layer), `lib/state-machine.ts`, worker retry/dead-letter/reaper. **Phase 3 complete.**

### This session's Phase 3 total (including the resumed continuation): commits `7a9afb8`, `f4e6e2e`, `5141f5b` (goals_status_check fix + state-machine.ts), `57df5bd` (worker retry/dead-letter/reaper + dispatch bug fix). Two live-production bugs found and fixed along the way that weren't part of the original plan (goals_status_check missing values; worker's dead auto-dispatch path) — both surfaced by reading real code before writing characterization tests/tables, not by assumption.
