...
export async function runOrcReport(goalId: string): Promise<void> {
  ...
  try {
    ...
    if (newReport) {
      ...
      // (.catch(() => {})) — the spec's declared source-of-truth table could
      // fail to receive a write with zero observability. Still
      // fire-and-forget (spec sanctions the dual-table design), but a real
      // failure is now logged instead of invisible.
      if (goal.created_by) {
        ...
        }).catch((err) => log.warn('[runOrcReport] company_memo dual-write (logDecision) failed', { module: 'engine/orchestrator', goalId, userId: goal.created_by, error: String(err) }))
      }
